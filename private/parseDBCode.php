...
function ciniki_systemdocs_parseDBCode($ciniki, $package, $module, $table) {
    ...
    for($i=0;$i<count($lines);$i++) {
        if( preg_match('/^\s*\#/', $lines[$i]) ) {
            ...
            elseif( preg_match('/\s*(?P<field>\S+)\s+(?P<type>longblob|mediumblob|blob|bigint|int|tinyint|smallint|mediumtext|text|datetime|date|time|numeric\(.*\)|char\([0-9]+\)|varchar\([0-9]+\)|decimal\(.*\))\s*(?P<unsigned>unsigned)?\s*(?P<null>\s|not null)?(?P<extras>.*)?,/', $lines[$i], $matches) ) {
